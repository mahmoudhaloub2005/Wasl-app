...
function getInitialFormValues(advertisement) {
  return {
    ...
    price:
      advertisement?.price === undefined || advertisement?.price === null
        ? ""
...
function AdvertisementForm({
  ...
  async function handleSelectedFile(file) {
    ...
    if (!["image/jpeg", "image/png"].includes(file.type)) {
      setUploadMessage("صيغة الصورة يجب أن تكون PNG أو JPG.");
      return;
    }

    if (file.size > MAX_UPLOAD_SIZE_BYTES) {
      setUploadMessage(`حجم الصورة يجب ألا يتجاوز ${MAX_UPLOAD_SIZE_MB}MB.`);
      return;
    }
    ...
    try {
      ...
      setUploadMessage("تعذر قراءة الصورة. حاول اختيار ملف آخر.");
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    const nextValidationMessages = validateForm(formValues);
    setValidationMessages(nextValidationMessages);

    if (nextValidationMessages.length) return;
    ...
    try {
      ...
      // The page-level alert shows the service error message.
    }
  }

  return (
    <section className="advertisement-form-card" aria-labelledby="ad-form-title">
      <h2 id="ad-form-title">
        {isEditing ? "تعديل الإعلان" : "إضافة إعلان جديد"}
      </h2>

      <form className="advertisement-form" onSubmit={handleSubmit}>
        <div className="advertisement-form__field advertisement-form__field--upload">
          <label htmlFor="advertisement-image">صورة الإعلان</label>
          <div
            className={`advertisement-form__upload ${
              formValues.imageUrl ? "advertisement-form__upload--has-image" : ""
            }`}
          ...
          >
            {formValues.imageUrl ? (
              <>
                <img src={formValues.imageUrl} alt="معاينة صورة الإعلان" />
                <button
                  type="button"
              ...
              <div className="advertisement-form__upload-placeholder">
                ...
                <span>تنسيقات PNG و JPG بحد أقصى 5MB</span>
              </div>
            )}
          </div>
          <input
          ...
          />
          {uploadMessage && (
            <p className="advertisement-form__field-message">{uploadMessage}</p>
          )}
        </div>

        <div className="advertisement-form__split">
          <div className="advertisement-form__field">
            <label htmlFor="advertisement-title">اسم العرض</label>
            <input
          ...
          <div className="advertisement-form__field">
            <label htmlFor="advertisement-price">السعر (شيكل)</label>
            <input
        ...
        <div className="advertisement-form__field">
          <label htmlFor="advertisement-description">وصف الإعلان</label>
          <textarea
        ...
        {validationMessages.length > 0 && (
          <div className="advertisement-form__errors" role="alert">
